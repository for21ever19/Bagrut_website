...
export default function SubjectPage() {
  ...
  // Получаем прогресс для отображения количества ошибок
  const { getSubjectProgress } = useProgress()
  const subjectProgress = getSubjectProgress(subjectId)
  const mistakesCount = subjectProgress?.wrongQuestionIds.length || 0
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {/* Карточка "Работа над ошибками" - показываем только если есть ошибки */}
            {mistakesCount > 0 && (
              <motion.div
                initial={{ opacity: 0, y: 30 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <Link href={`/subject/${subjectId}/train?mode=mistakes`} className="w-full max-w-md">
                  <motion.div
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4 relative z-10">
                      <RefreshCcw className="w-8 h-8 text-ink flex-shrink-0 mt-1" />
                      <div className="flex-1">
                        <h2 className="text-2xl font-serif font-bold text-ink mb-2">
                          Работа над ошибками ({mistakesCount})
                        </h2>
                        <p className="text-ink opacity-70 text-lg">
                          Повторите вопросы, на которые вы ответили неправильно
                        </p>
                      </div>
                    </div>
                  </motion.div>
